...
function vtkInteractorStyleMPRWindowLevel(publicAPI, model) {
  ...
  model.trackballManipulator = windowLevelManipulator.newInstance({
    button: 1,
  });
  ...
  publicAPI.setVolumeMapper = mapper => {
    model.trackballManipulator.setVolumeMapper(mapper);
...
export function extend(publicAPI, model, initialValues = {}) {
  ...
  macro.setGet(publicAPI, model, ['volumeMapper', 'levelScale']);
