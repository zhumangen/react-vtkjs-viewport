...
function vtkInteractorStyleMPRSlice(publicAPI, model) {
  ...
  publicAPI.updateScrollManipulator = () => {
    ...
    const btn = model.scrollManipulator.getButton();
    if (btn === 1) {
      model.scrollManipulator.removeHorizontalListener();
      model.scrollManipulator.setVerticalListener(
        range[0],
        range[1],
        1,
        publicAPI.getSlice,
  ...
  function notifyScrollChange() {
    const range = publicAPI.getSliceRange();
    const onScrollChanged = publicAPI.getOnScrollChanged();
    if (onScrollChanged) {
      onScrollChanged({
        min: range[0],
        max: range[1],
        value: model.interactor ? publicAPI.getSlice() : undefined,
        set: publicAPI.setSlice,
      });
    }
  }
  ...
  publicAPI.handleMouseMove = macro.chain(publicAPI.handleMouseMove, () => {
    const voi = publicAPI.getWindowLevel();
  ...
  publicAPI.getWindowLevel = () => {
    if (model.volumeMapper && model.volumeMapper.getProperty) {
  ...
  publicAPI.setSlice = slice => {
    ...
    if (model.volumeMapper) {
      ...
      if (model.sliceCenter.length) {
        const oldPos = camera.getFocalPoint();
        vtkMath.subtract(oldPos, model.sliceCenter, cameraOffset);
      }
